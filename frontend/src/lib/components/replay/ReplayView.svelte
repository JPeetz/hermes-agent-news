<script lang="ts">
	...
	async function ensureItemIndex() {
		if (itemIndexState !== 'idle' || !index) return;
		itemIndexState = 'loading';
		const idxDate = index.date;
		const categories: Category[] = ['news', 'research', 'social', 'reddit'];
		const results = await Promise.allSettled(categories.map((c) => loadCategoryData(idxDate, c)));
	...
	$: if (browser && date && date !== lastLoadedKey) {
		lastLoadedKey = date;
		void boot(date);
	}
	...
	async function boot(key: string) {
		...
		try {
			if (!key) throw new Error('No reports available yet.');
			index = await loadReplayIndex(key);
			attach(index);
		} catch (e) {
			index = null;
	...
	{:else if loadError || !index}
		<div class="card text-center py-14 px-6">
			...
			<div class="mt-5 flex flex-wrap items-center justify-center gap-3">
				<a href="/?date={date ?? ''}" class="btn-secondary text-sm">Back to the report</a>
			</div>
		</div>
	{:else if frame}
		<!-- Run header stats -->
