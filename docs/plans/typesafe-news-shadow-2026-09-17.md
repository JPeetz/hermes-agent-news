...
| Filter control | 900 s/request, three total attempts, matched 65,536-token production output ceiling; a shared 3,600 s budget includes the repeated control |
| Judge | At most two concurrent requests; 900 s/request, up to three attempts for retryable failures; 384,000 output tokens/request, 64 requests and 1,920,000 aggregate output tokens/date, with a 3,600 s stage deadline |
...
The judge limit was updated after the first hosted legacy comparison (run `35227530535`) exhausted its 16,384-token response allowance during DeepSeek reasoning. The authenticated RDSec `/v1/model/info` catalog verified `deepseek-v4.1-flash` at 384,000 maximum output tokens on September 17, 2026. The judge now uses that maximum without changing reasoning behavior; its aggregate budget covers five full 16-item batches at that ceiling. Length-terminated responses are saved as incomplete rather than retried unchanged. Candidate thresholds remain unchanged.

The first repeated control also reused the original control's completion ID. Shadow RDSec requests now use the gateway's [cache bypass controls](https://docs.litellm.ai/docs/proxy/caching_controls), `cache: {"no-cache": true, "no-store": true}`, while preserving frozen prompt messages. Repeated or missing completion IDs disqualify repeatability evidence; distinct completion IDs show only that a duplicate completion was not observed. Gateway request IDs are recorded separately. Corrected evaluations receive a new experiment/retry identity and preserve the original degraded result.

Hosted runs emit flushed, structured progress to stderr at each stage/request start and completion, plus a heartbeat every 30 seconds while waiting. Events contain stage/model identifiers, item counts, elapsed time, completion status and measured input/output tokens only; prompts, model response text, headers and credentials are excluded. Model calls remain non-streaming, and stdout remains the final machine-readable result. The completed corrected trial (`35230127782`) covered all 78 items with five successful judge requests; both control calls recovered from 240-second timeouts, so subsequent control calls use 900 seconds within the existing aggregate deadline.
